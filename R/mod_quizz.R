...
mod_quizz_server <- function(id, rv_global) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(rv_global$flag_question, {
      ...
      rv_global$flag_other <- sample_country(
        list_country = rv_global$world %>%
          filter(country != rv_global$flag_question$name),
        n = 2,
        url_flag = FALSE
      )
      # concatenation du flag a questionner avec les other flag
      rv_global$flag <- mapply(
        c,
        rv_global$flag_question,
        rv_global$flag_other
      )
    })
    ...
    observeEvent(input$reponse_select, {
      ...
      rv_global$flag_memory <- dplyr::bind_rows(rv_global$flag_memory,
                                                rv_global$flag_question[1])
      
      rv_global$flag_question <- sample_country(
        # on repropose un drapeau parmi ceux qui non pas ete propose
        list_country = dplyr::anti_join(rv_global$world,
                         rv_global$flag_memory,
                         by = c("country" = "name")),
        1,
        url_flag = TRUE
      )
